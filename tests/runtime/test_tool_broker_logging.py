...
@pytest.mark.asyncio
async def test_tool_broker_logs_success_with_arguments_and_result_snapshot() -> None:
    ...
    try:
        ...
        assert item["message"] == "Tool executed"
        assert item["extra"]["tool_name"] == "echo"
        assert item["extra"]["run_id"] == "run:1"
        assert item["extra"]["thread_id"] == "qq:user:10001"
        assert item["extra"]["agent_id"] == "agent:frontstage"
        assert item["extra"]["actor_id"] == "qq:user:10001"
        assert item["extra"]["tool_arguments"]["url"] == "https://example.com"
        assert item["extra"]["tool_arguments"]["token"] == "[REDACTED]"
        snapshot_payload = item["extra"]["tool_result_snapshot"]
...
async def test_tool_broker_logs_failure_with_error_and_arguments() -> None:
    ...
    try:
        ...
        assert item["message"] == "Tool execution failed"
...
async def test_tool_broker_logs_rejection_with_reason_and_arguments() -> None:
    ...
    try:
        ...
        assert item["message"] == "Tool rejected"
