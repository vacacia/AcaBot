...
- 当前代码已经完成这条线的硬切。
- 现在前台正式真源是 `session.yaml + agent.yaml`，不再走共享前台 agent 注册表模型。
- 当前模型已经收成“`session` 绑定自己专属的独立 `agent` 对象”。
...
- `session.yaml` 不再持有旧的前台 profile 字段。命名正式收成 `frontstage_agent_id`。
...
- 第一阶段就正式把旧词收掉:
  - 旧的前台 profile 入口统一收成 `frontstage_agent_id`
  - 旧的 routing/work-world 标识统一收成 `agent_id`
