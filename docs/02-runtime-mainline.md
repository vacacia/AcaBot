...
12. pipeline 收尾，保存 thread，结束 run，触发长期记忆提取
...
6. `resolve_context(...)`
   - 算 retrieval / sticky note / prompt slot 相关输入
...
8. 准备 retrieval plan
9. 注入长期记忆和文件记忆
10. 把当前轮真正给模型看的 message 应用到 `ctx.messages`
11. 跑 `PRE_AGENT` hooks
12. 调 `agent_runtime.execute(ctx)`
...
- compaction 后的工作记忆
- retrieval planner 准备的消息列表
- memory broker 注入的长期记忆
- 当前轮消息整理后的 model message
