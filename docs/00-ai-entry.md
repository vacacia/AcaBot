...
# 编写代码的规范
0. 任何注释都要用直白的语言, 说人话! 不要出现黑话
1. 类的 docstring 用 google 风格, 并且要包含属性的类型
2. 文档级别的 docstring 描述文件的功能, 和其他组件的联系
3. 方法的 docstring 要写清楚输入输出, 和方法的功能
  - 任何方法都要有 docstring, 哪怕是`__init__` 和 helper 
4. 用简洁的中文加上 `# region `注释,方便定位
5. 每次完成一点后, 要详细解释这次是做了什么, 为什么这样做, 为什么不那样做?站在设计师的视角解释
6. 完成一次commit之后, 不要提交, 给出 git add 的文件列表, git commit 信息, 和你修改的内容, 你做的决策, 目前的流程...全部直白的解释清楚, 不准 commit!!! 然后立即启动子代理 (gpt5.4) 用 superpowers 去 review 你这次的代码 和 测试文件
7. 发现难以决策的点就停下来, 说清楚现状, 及时讨论
8. 清晰的代码目录结构, 相关的代码文件放在一个文件夹下面; 明确的代码文件夹命名, 不要用含糊不清的文件夹名;
...
这篇文档记录的是当前已经达成的共识。它不替代系统地图和主线说明，但会先告诉你：
