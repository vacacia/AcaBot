...
Reconciler 不做定时轮询。如果一个插件加载失败（比如外部依赖暂时不可用），它会一直处于 `failed` 状态，直到操作者手动改 Spec 或点"重新扫描"触发 `reconcile_all()`。这是有意的选择——当前阶段插件数量少，自动重试是过度工程。以后插件多起来或接入定时任务基础设施后，可以加定时 reconcile。
