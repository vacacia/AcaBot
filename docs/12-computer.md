...
Computer 是 AcaBot 为 AI 模型提供的受控文件系统和执行环境。模型侧正式工作语义是世界路径（World Path）：`/workspace/...`、`/skills/...`、`/self/...`。前台 `read` / `write` / `edit` 围绕这些 world path 工作；`bash` 也在同一轮 workspace 契约下运行，但在 host backend 下 shell 仍可能暴露真实宿主机路径。所以 `/workspace` 是模型侧语义，不等于 host shell 的完全路径虚拟化。模型不直接看到 `computer` 这个名字，看到的是四个 builtin tool——`read`、`write`、`edit`、`bash`。Computer 决定了每一轮 run 中模型能看到什么、能做什么。
...
对模型来说，“所有工作都在 `/workspace` 中完成”是稳定合同：整理输入文件、生成待发送附件、读写中间产物都应先落在 `/workspace`。如果模型要通过 `message` tool 主动向 QQ 发送本地文件，也必须先把文件准备到 `/workspace`，再使用相对路径引用。这个 QQ 本地文件规则属于 message tool contract，不属于 Computer 对 host shell 的路径虚拟化。

render artifacts 不走 Work World。`message.send.render` 在 Outbox 里生成的图片落到 runtime internal `runtime_data/render_artifacts/`，不进入 `/workspace/attachments/`，也不会作为 Computer 可见文件暴露给模型。
...
也就是说，host backend 下 shell 能看到真实路径是当前实现的一部分，不和“模型工作语义在 `/workspace`”矛盾；Phase 7 也不试图把 host shell 额外伪装成 `/workspace` 视图。
...
`message.send.render` 生成的图片不走这条附件 staging 线。它属于 runtime 内部发送 artifact，由 render service 直接写到 `runtime_data/render_artifacts/`，只给 Outbox 发消息时消费。
