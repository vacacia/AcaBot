...
> **这一轮该查什么、当前保留了哪些历史、当前 working summary 是什么。**
...
run 结束后，`MemoryBroker.extract_after_run(ctx)` 再负责长期记忆写回。
...
9. `BaseAgent.run(...)`
10. `MemoryBroker.extract_after_run(ctx)`
...
- `MemoryBroker` 统一读取 `/self`、sticky note、长期记忆，并负责 run 后写回
