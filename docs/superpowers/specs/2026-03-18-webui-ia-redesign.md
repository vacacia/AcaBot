...
Bot 页面负责默认配置，不负责编辑具体某个 Session。
...
- Skills 从已安装 skill 列表里勾选
- 只展示实际存在的 skill

### Bot / 默认输入处理

这一块继续沿用“默认行为”的概念，但 UI 上仍以直接可读的输入处理规则呈现，而不是暴露底层 rule id。
...
会话页负责查看和编辑某个具体 Session，但第一版只覆盖当前代码里已经有清楚真源的部分。
...
- 标签页切换必须真实可用
- `基础信息 / Agent 绑定 / 输入处理` 点击后应切换到对应面板
...
- Session 只管自己的设置，底层最后再抽象出 agent

## Session / Agent 绑定

这一页只负责 Session 范围内的绑定设置。
...
- 允许编辑按 `channel_scope` 命中的 binding rule
- 不在这一页直接创造“Session 自己的一套 AI 配置”
- 这一页管理的是 Session 的绑定设置，不是展示一个“当前 agent 面板”

## Session / 输入处理

这一页回答：不同输入类型进入这个会话时，怎么处理、怎么存。

这里不是配置“会话是私聊还是群聊”，而是配置各种输入类型的处理规则。

支持的输入处理规则全部列出，每条规则默认折叠。
...
前端展示的是当前生效值，不把底层 binding / inbound / event policy 三套对象作为主界面概念。
...
- 前端测试
  ...
  - Session 的 `基础信息 / Agent 绑定 / 输入处理` 标签页切换正常
