...
在 `src/acabot/runtime/tool_broker.py`。
...
- profile 通过 `skill_assignments` 决定当前 agent 能看到哪些 skill

也就是说，在 AcaBot 这里:

- skill 先是“文档化的能力包”
- 然后才通过 catalog、assignment、delegation 接进运行时
...
AcaBot 里也是 skill package，但它还多了运行时这一层:
...
- skill 不只是被读
- 还会被谁看到、是否允许委派、委派给哪个 subagent

所以在这个项目里，skill 不是单纯提示词片段，而是“可被配置、分配、委派的能力包”。
...
4. 再决定是否要做更自动的 skill 触发
   也就是不一定每次都显式 `skill(name=...)`

如果按这个方向走，那 AcaBot 的 skill 更像:

- 直接兼容外部 agent skill 的文件格式
- 再在 runtime 上补一层 assignment、delegation 和可见性控制
...
一个 agent 的 profile 里可以有 `skill_assignments`，其中会带:

- `skill_name`
- `delegation_mode`
- `delegate_agent_id`

如果 profile 有 delegated skills，或系统里 profile 不止一个，bootstrap 会把 `SubagentDelegationPlugin` 接上。
...
### 1. direct subagent delegation 现在偏宽松

当前 `delegate_subagent` 支持直接传 `delegate_agent_id`。

这条路径现在更像“只要 registry 里有这个 executor，就能直接调”，没有完全收紧到:

- 当前 profile 是否真的可见这个 subagent
- 是否应该受 `skill_assignments` / delegation policy 约束

所以如果以后你要修 subagent 可见性或能力隔离，这里要同步更新。

### 2. skill 现在更像“可读能力包”，还不是完整外部 skill 运行时
...
所以如果以后 AcaBot 真正能直接吃外部 skill，这一段也要改。
...
1. `src/acabot/runtime/tool_broker.py`
2. `src/acabot/runtime/model_agent_runtime.py`
3. `src/acabot/runtime/plugin_manager.py`
4. `src/acabot/runtime/plugins/`
5. `src/acabot/runtime/subagent_delegation.py`
6. `src/acabot/runtime/subagent_execution.py`
